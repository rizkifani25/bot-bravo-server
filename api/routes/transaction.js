...
router.get('/', async (req, res, next) => {
    ...
    if (trxId != null && trxCode != null && trxTo != null && trxNominal != null && device != null ) {
        Transaction.find({
            'trxId': trxId
        }, { _id: 0, __v:0 })
            .exec()
            .then(docs => {
                if (docs.length == 1) {
                    console.log("Transaction with trxId " + trxId + " exist. Showing data.")
                    res.status(200).json(docs)
                } else {
                    console.log("Transaction with trxId " + trxId + " doesn't exist. Do new transaction.")
